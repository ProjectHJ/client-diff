package com.junkbyte.console.view
{
   ...
   public class ConsolePanel extends Sprite
   {
      ...
      protected function init(param1:Number, param2:Number, param3:Boolean = false, param4:Number = -1, param5:Number = -1, param6:int = -1) : void
      {
         ...
         this.bg.graphics.beginFill(param4 >= 0?uint(param4):uint(this.style.backgroundColor),param5 >= 0?Number(param5):Number(this.style.backgroundAlpha));
         if(param6 < 0)
         {
      ...
      private function updateScale(param1:Event = null) : void
      {
         ...
         _loc2_.x = _loc2_.x - x;
         _loc2_.y = _loc2_.y - y;
         this.width = _loc2_.x < this.minWidth?Number(this.minWidth):Number(_loc2_.x);
         this.height = _loc2_.y < this.minHeight?Number(this.minHeight):Number(_loc2_.y);
         this.updateScaleText();
      }
