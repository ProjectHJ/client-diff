package com.company.assembleegameclient.objects
{
   ...
   public class Player extends Character
   {
      ...
      protected function drawBreathBar(param1:Vector.<IGraphicsData>, param2:int) : void
      {
         ...
         {
            _loc7_ = (Parameters.BREATH_THRESH - this.breath_) / Parameters.BREATH_THRESH;
            this.breathBackFill_.color = MoreColorUtil.lerpColor(5526612,16711680,Math.abs(Math.sin(param2 / 300)) * _loc7_);
         }
         else
         {
         ...
         _loc6_.length = 0;
         _loc6_.push(posS_[0] - _loc3_,posS_[1] + _loc4_,posS_[0] + _loc3_,posS_[1] + _loc4_,posS_[0] + _loc3_,posS_[1] + _loc4_ + _loc5_,posS_[0] - _loc3_,posS_[1] + _loc4_ + _loc5_);
         param1.push(this.breathBackFill_);
         param1.push(this.breathBackPath_);
         param1.push(GraphicsUtil.END_FILL);
         if(this.breath_ > 0)
         {
            _loc8_ = this.breath_ / 100 * 2 * _loc3_;
            this.breathPath_.data.length = 0;
            _loc6_ = this.breathPath_.data as Vector.<Number>;
            _loc6_.length = 0;
            _loc6_.push(posS_[0] - _loc3_,posS_[1] + _loc4_,posS_[0] - _loc3_ + _loc8_,posS_[1] + _loc4_,posS_[0] - _loc3_ + _loc8_,posS_[1] + _loc4_ + _loc5_,posS_[0] - _loc3_,posS_[1] + _loc4_ + _loc5_);
      ...
      public function useAltWeapon(param1:Number, param2:Number, param3:int) : Boolean
      {
         ...
         {
            if(_loc7_.toString() == ActivationType.TELEPORT)
            {
               if(!this.isValidPosition(_loc6_.x,_loc6_.y))
               {
                  SoundEffectLibrary.play("error");
                  return false;
               }
         ...
         if(param3 == UseType.START_USE)
         {
            ...
            this.nextAltAttack_ = _loc8_ + _loc11_;
            map_.gs_.gsc_.useItem(_loc8_,objectId_,1,_loc4_,_loc6_.x,_loc6_.y,param3);
            if(_loc5_.Activate == ActivationType.SHOOT)
            {
               _loc9_ = Math.atan2(param2,param1);
               this.doShoot(_loc8_,_loc4_,_loc5_,Parameters.data_.cameraAngle + _loc9_,false);
            }
         }
      ...
      private function doShoot(param1:int, param2:int, param3:XML, param4:Number, param5:Boolean) : void
      {
         ...
         var _loc16_:int = 0;
         var _loc6_:int = !!param3.hasOwnProperty("NumProjectiles")?int(int(param3.NumProjectiles)):1;
         var _loc7_:Number = (!!param3.hasOwnProperty("ArcGap")?Number(param3.ArcGap):11.25) * Trig.toRadians;
         ...
         {
            _loc11_ = getBulletId();
            _loc12_ = FreeList.newObject(Projectile) as Projectile;
            if(param5 && this.projectileIdSetOverrideNew != "")
            {
               _loc12_.reset(param2,0,objectId_,_loc11_,_loc9_,param1,this.projectileIdSetOverrideNew,this.projectileIdSetOverrideOld);
            }
            else
            {
               _loc12_.reset(param2,0,objectId_,_loc11_,_loc9_,param1);
            }
            _loc13_ = int(_loc12_.projProps_.minDamage_);
            _loc14_ = int(_loc12_.projProps_.maxDamage_);
            _loc15_ = !!param5?Number(this.attackMultiplier()):Number(1);
            _loc16_ = map_.gs_.gsc_.getNextDamage(_loc13_,_loc14_) * _loc15_;
            if(param1 > map_.gs_.moveRecords_.lastClearTime_ + 600)
            {
               _loc16_ = 0;
            }
            _loc12_.setDamage(_loc16_);
            if(_loc10_ == 0 && _loc12_.sound_ != null)
            {
               SoundEffectLibrary.play(_loc12_.sound_,0.75,false);
            }
