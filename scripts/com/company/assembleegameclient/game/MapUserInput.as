package com.company.assembleegameclient.game
{
   ...
   public class MapUserInput
   {
      ...
      {
         var _loc4_:AddTextLineSignal = null;
         ...
         if(this.currentString == UIUtils.EXPERIMENTAL_MENU_PASSWORD.slice(0,this.currentString.length))
         {
            ...
            {
               _loc4_ = StaticInjectorContext.getInjector().getInstance(AddTextLineSignal);
               _loc5_ = new ChatMessage();
               _loc5_.name = Parameters.SERVER_CHAT_NAME;
               this.currentString = "";
               UIUtils.SHOW_EXPERIMENTAL_MENU = !UIUtils.SHOW_EXPERIMENTAL_MENU;
               _loc5_.text = !!UIUtils.SHOW_EXPERIMENTAL_MENU?"Experimental menu activated":"Experimental menu deactivated";
               _loc4_.dispatch(_loc5_);
            }
         }
         else
         {
         ...
         switch(param1.keyCode)
         {
            ...
            case Parameters.data_.useSpecial:
               ...
               if(!this.specialKeyDown_)
               {
                  ...
                  {
                     _loc7_ = Math.random() * 600 - 300;
                     _loc8_ = Math.random() * 600 - 325;
                  }
                  else
                  {
                     _loc7_ = this.gs_.map.mouseX;
                     _loc8_ = this.gs_.map.mouseY;
                  }
                  _loc9_ = _loc3_.useAltWeapon(_loc7_,_loc8_,UseType.START_USE);
                  if(_loc9_)
                  {
         ...
         if(this.areFKeysAvailable)
         {
            switch(param1.keyCode)
            {
               case KeyCodes.F6:
                  ...
                  break;
               case KeyCodes.F7:
                  for each(_loc10_ in this.gs_.map.squares_)
                  {
                     if(_loc10_ != null)
                     {
